...
class App {
  constructor() {
    ...
    this.geoFields.on('change', 'update', this);
  }
  hookup() {
    const me = this
    me.geoFields.on('change', 'update', this)
    me.geoApi.on('change', 'update', this)
    $('#geocode').click(() => {
      me.getData(true)
    })
  ...
  setup() {
    if (Conf.valid()) {
      ...
      if (Conf.get('nyc')) {
        this.sheetGeocoder.projection = 'EPSG:2263'
        this.map = new Basemap({target: 'map', layers: [layer]})
        geocoder = new Geoclient({})
      } else {
        this.sheetGeocoder.projection = ''
        this.map = new Map({
          target: 'map',
          view: new ol.View({center: Basemap.CENTER, zoom: 10}),
          layers: [new ol.layer.Tile({source: new ol.source.OSM()}), layer]
        })
        geocoder = new CensusGeocoder()
      }
      this.popup = new Popup({map: map})
      this.locationMgr = new LocationMgr({map: map, geocoder: geocoder})
      this.sheetGeocoder.conf(Conf.get())
      this.locationMgr.locator.geocoder.url = Conf.get('url')
      this.locationMgr.on('geocoded', $.proxy(this.showPopup, this))
